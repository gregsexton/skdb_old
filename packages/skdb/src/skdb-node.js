...
export async function fetchWasmSource() {
    ...
    let wasmBuffer = fs.readFileSync(new URL("./skdb.wasm", import.meta.url));
...
function encodeProtoMsg(msg) {
    switch (msg.type) {
        ...
        case "tail": {
            ...
            encodeResult = textEncoder.encodeInto(msg.filterExpr, uint8View.subarray(filterExprOffset + 2));
...
class ResilientMuxedSocket {
    ...
    isSocketErrorRetryable(errorCode) {
        if (errorCode === 1002) {
...
class SKDBServer {
    ...
    async establishServerTail(tableName, filterExpr) {
    ...
    async mirrorTable(tableName, filterExpr) {
